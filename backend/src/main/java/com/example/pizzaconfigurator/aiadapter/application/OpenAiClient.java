...
/** Fallback provider (agent.md §7.4, ADR-008), tried only after Deepseek fails or times out. */
@Component
class OpenAiClient implements AiProviderClient {
    ...
    OpenAiClient(
        ...
        @Value("${app.ai.openai.timeout-ms}") long timeoutMs,
    ...
    public RawParsedComment interpret(String comment, ConfigurableOptions allowedOptions) {
        if (apiKey.isBlank()) {
            throw new AiProviderException(name() + " is not configured (OPENAI_API_KEY unset)");
        }
