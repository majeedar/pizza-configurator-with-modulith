...
class DoughCompatibilityEvaluatorTest {

    private final DoughCompatibilityEvaluator evaluator = new DoughCompatibilityEvaluator(JSON);
    private final RuleDefinition napoliNoGlutenFree =
        rule("NAPOLI_NO_GLUTEN_FREE", RuleType.DOUGH_COMPATIBILITY, "{\"doughCode\":\"GLUTEN_FREE\",\"allowed\":false}");
    private final RuleEvaluationContext context = context("NAPOLI");

    @Test
    void allowedDoughIsValid() {
        ConfigurationCandidate candidate = candidate("M", "CLASSIC", Set.of());

        assertThat(evaluator.evaluate(napoliNoGlutenFree, candidate, context)).isEmpty();
    }

    @Test
    void disallowedDoughIsInvalid() {
        ConfigurationCandidate candidate = candidate("M", "GLUTEN_FREE", Set.of());

        List<Violation> violations = evaluator.evaluate(napoliNoGlutenFree, candidate, context);

        assertThat(violations).hasSize(1);
        assertThat(violations.get(0).code()).isEqualTo("DOUGH_NOT_AVAILABLE");
    }
}
